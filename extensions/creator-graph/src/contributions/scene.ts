...
exports.methods = {
    async queryShaderNode() {
        const { shaderNodeMap, shaderPropertyMap } = await Editor.Module.importProjectModule('db://creator-graph/graph/index.ts') as IModuleOptions;
    ...
    async queryPropertyValueDumpByType(type: string, value: any) {
        const { shaderPropertyMap } = await Editor.Module.importProjectModule('db://creator-graph/graph/index.ts') as IModuleOptions;
