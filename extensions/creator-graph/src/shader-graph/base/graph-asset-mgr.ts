...
export class GraphAssetMgr {
    ...
    public async open(): Promise<boolean> {
        try {
            const result = await Editor.Dialog.select({
                title: Editor.I18n.t('creator-graph.messages.titles.open'),
                path: PROJECT_PATH,
                type: 'file',
                multi: false,
    ...
    public async createNew(type: string): Promise<boolean> {
        try {
            const result = await Editor.Dialog.save({
                title: Editor.I18n.t('creator-graph.messages.save.title'),
    ...
    public async checkIfSave(): Promise<boolean> {
        if (this.shaderGraphAssetInfo && !existsSync(this.shaderGraphAssetInfo.file)) {
            const result = await Editor.Dialog.warn(Editor.I18n.t('creator-graph.messages.missing_assets.detail'), {
                title: Editor.I18n.t('creator-graph.messages.titles.normal'),
                default: 0,
                cancel: 1,
                buttons: [
                    Editor.I18n.t('creator-graph.buttons.save'),
                    Editor.I18n.t('creator-graph.buttons.unsaved'),
                ],
            });
            ...
            const result = await Editor.Dialog.warn(Editor.I18n.t('creator-graph.messages.save.detail'), {
                title: Editor.I18n.t('creator-graph.messages.titles.normal'),
                default: 0,
                cancel: 1,
                buttons: [
