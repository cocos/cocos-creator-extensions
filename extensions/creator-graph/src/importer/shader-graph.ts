...
export class ShaderGraph {
    ...
    async initGraph() {
        ...
        await Editor.Module.importProjectModule('db://creator-graph/operation/index.ts');

        const { ShaderProperty } = await Editor.Module.importProjectModule('db://creator-graph/operation/property.ts') as any;
        this.ShaderProperty = ShaderProperty;

        const { shaderNodeMap, shaderPropertyMap, shaderNodeClassMap } = await Editor.Module.importProjectModule('db://creator-graph/graph/index.ts') as IModuleOptions;

        const { shaderContext } = await Editor.Module.importProjectModule('db://creator-graph/operation/context.ts') as any;
        declareShaderNodeBlock(shaderNodeMap);

        this.shaderNodeClassMap = shaderNodeClassMap;
        this.shaderContext = shaderContext;

        shaderContext.shaderTemplatesDir = await Editor.Message.request('asset-db', 'query-path', 'db://creator-graph/../compile-shader/shader-templates') as string;
        // shaderContext.shaderTemplatesDir = queryPath('db://creator-graph/../compile-shader/shader-templates');
